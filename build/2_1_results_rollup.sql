...
SELECT      personId,
            ...
            CAST(SUM(IF(roundTypeId IN ('C', 'F'), 1, 0)) AS UNSIGNED INT) AS numFinal,
            CAST(SUM(IF(roundTypeId IN ('C', 'F') AND pos <= 3 AND best > 0, 1, 0)) AS UNSIGNED INT) AS numPodium,
            CAST(COUNT(roundTypeId) AS UNSIGNED INT) AS numRound,
