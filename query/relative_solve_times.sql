-- 444 VS 333 (World) - Approach 1 uses the SPV
SELECT      CEIL(worldCentileAverage444 / 5.0) * 5 AS vigintile, COUNT(*) AS numPersons,
            TRUNCATE(AVG(bestAverage444) / 100.0, 2) AS avg444, TRUNCATE(AVG(bestAverage333) / 100.0, 2) AS avg333,
...
-- 444 VS 333 (World) - Approach 2 uses the ranks pivot table
SELECT      CEIL(worldCentile444 / 5.0) * 5 AS vigintile, COUNT(*) AS numPersons,
            TRUNCATE(AVG(best444) / 100.0, 2) AS avg444, TRUNCATE(AVG(best333) / 100.0, 2) AS avg333,
...
-- 444 VS 333 (UK) - Approach 1 uses the SPV
...
-- 444 VS 333 (UK) - Approach 2 uses the ranks pivot tabl, joined back to the persons table
